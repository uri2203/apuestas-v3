...
MAX_EXPOSURE_POR_PARTIDO = 5
MAX_EXPOSURE_POR_DEPORTE = 20
MAX_APUESTAS_DIARIAS = 15
STOP_LOSS_DIARIO = -500
STOP_LOSS_SEMANAL = -2000
MAX_DRAWDOWN_PCT = 15
    ...
    try:
        ...
        with db() as conn:
            ...
            if today_count >= MAX_APUESTAS_DIARIAS:
                alerts.append(f"Limite diario alcanzado ({today_count}/{MAX_APUESTAS_DIARIAS})")
            ...
            if drawdown_pct >= MAX_DRAWDOWN_PCT:
                alerts.append(f"DRAWDOWN MAXIMO: {drawdown_pct:.1f}%")
            if bankroll < MIN_BANKROLL_RESERVE:
                alerts.append(f"Bankroll bajo reserva: ${bankroll:.2f}")
...
def check_can_bet(sport: str, match: str, stake: float) -> dict:
    ...
    try:
        ...
        if today_count >= MAX_APUESTAS_DIARIAS:
            allowed = False
            reasons.append(f"Limite diario: {today_count}/{MAX_APUESTAS_DIARIAS}")

        match_pct = ((match_staked + stake) / bankroll * 100) if bankroll > 0 else 100
        if match_pct > MAX_EXPOSURE_POR_PARTIDO:
            allowed = False
            reasons.append(f"Exposicion por partido: {match_pct:.1f}% > {MAX_EXPOSURE_POR_PARTIDO}%")

        sport_pct = ((sport_staked + stake) / bankroll * 100) if bankroll > 0 else 100
        if sport_pct > MAX_EXPOSURE_POR_DEPORTE:
            allowed = False
            reasons.append(f"Exposicion por deporte: {sport_pct:.1f}% > {MAX_EXPOSURE_POR_DEPORTE}%")
...
def update_risk_config(config: dict) -> dict:
    ...
    try:
        if "max_exposure_por_partido" in config:
            MAX_EXPOSURE_POR_PARTIDO = max(0, min(100, float(config["max_exposure_por_partido"])))
        if "max_exposure_por_deporte" in config:
            MAX_EXPOSURE_POR_DEPORTE = max(0, min(100, float(config["max_exposure_por_deporte"])))
        if "max_apuestas_diarias" in config:
            MAX_APUESTAS_DIARIAS = max(1, min(100, int(config["max_apuestas_diarias"])))
        ...
        if "max_drawdown_pct" in config:
            MAX_DRAWDOWN_PCT = max(0, min(100, float(config["max_drawdown_pct"])))
        if "max_kelly_fraction" in config:
            MAX_KELLY_FRACTION = max(0.01, min(1.0, float(config["max_kelly_fraction"])))
        if "min_bankroll_reserve" in config:
            MIN_BANKROLL_RESERVE = max(0, float(config["min_bankroll_reserve"]))
    except (ValueError, TypeError) as e:
        logger.error("Error updating risk config: %s", e)
